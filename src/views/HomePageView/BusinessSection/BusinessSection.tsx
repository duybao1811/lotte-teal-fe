...
const BusinessSection = () => {
  ...
  return (
    <div className="w-full" ref={sectionRef}>
      <div className="flex justify-between mb-12 max-w-content px-4 sm:px-6 lg:px-8 gap-6">
        <motion.div
          ...
          className={'text-left'}
        >
          <h3 className="md:text-4xl sm:text-3xl text-2xl mb-2 font-extrabold md:whitespace-normal whitespace-pre-line">{`LOTTE \n ALUMINUM`}</h3>
          <h2 className="text-primary md:text-3xl sm:text-2xl text-xl font-extrabold">
            Businesses
          </h2>
        </motion.div>

        <motion.p
          className="whitespace-pre-line flex-1 flex justify-center md:text-base text-sm"
          initial={{ x: 80, opacity: 0 }}
          animate={isInView ? { x: 0, opacity: 1 } : {}}
          transition={{ duration: 0.8, ease: 'easeOut' }}
        >
      ...
      {isTablet ? (
        <>
          ...
          />
          <BusinessCarouselScroll
            data={extendData}
            activeIndex={itemActive}
            onChangeIndex={setItemActive}
          />
        </>
      ) : (
        <>
